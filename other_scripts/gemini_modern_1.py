...
ideal_fct = base_rtt + message_size / (125 * 400)  
...
for ecn_value in ecn_absolute_values:
    ...
    cmd = (
        ...
        f"-topology interdc -os_border 1 -strat rand -linkspeed 400000 "
        f"-topo lcp/configs/topos/fat_tree_400Gbps_gemini.topo -tm lcp/configs/tms/simple/gemini_intradc.cm "
        ...
        f"-mdRTT 0.0003515625 -interdcDelay 886500 -forceQueueSize {queue_Size * 1} -end_time 8000 -mtu 1500 -dctcp -forceKmin {ecn_value * mtu_size} -forceKmax {ecn_value * mtu_size} -forceCWND 15000 > tmp.tmp"
    )
    print(f"Running: {cmd}")
