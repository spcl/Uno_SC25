...
int FatTreeInterDCTopology::inter_dc_kmin = -1;
int FatTreeInterDCTopology::inter_dc_kmax = -1;
bool FatTreeInterDCTopology::use_inter_dc_ecn = false;
double FatTreeInterDCTopology::random_drop_perc = -1;
...
double FatTreeInterDCTopology::_percentage_link_down = 0;
bool FatTreeInterDCTopology::_fail_one = false;
...
FatTreeInterDCTopology *FatTreeInterDCTopology::load(istream &file, QueueLoggerFactory *logger_factory,
    ...
    do {
        ...
        if (tokens.size() == 0 || tokens[0][0] == '#') {
            ...
            cout << "Current tier: Tier" << current_tier << endl;
            ...
            cout << "  Radix_Up: " << _radix_up[current_tier] << endl;
            ...
            cout << "  Radix_Down: " << _radix_down[current_tier] << endl;
            ...
            cout << "  Oversubscribed: " << _oversub[current_tier] << endl;
            ...
            cout << "  Switch_Latency: " <<  _switch_latencies[current_tier] << " ps" << endl;
            ...
            cout << "  Downlink_Latency: " <<  _link_latencies[current_tier] << " ps" << endl;
        } else {
...
FatTreeInterDCTopology::FatTreeInterDCTopology(uint32_t no_of_nodes, linkspeed_bps linkspeed, mem_b intra_queuesize, mem_b inter_queuesize,
                                               ...
                                               queue_type q, simtime_picosec link_latency, simtime_picosec switch_latency,
                                               queue_type snd) {
    ...
    cout << "Fat Tree topology with " << timeAsUs(_link_latency) << "us links and " << timeAsUs(_switch_latency)
...
FatTreeInterDCTopology::FatTreeInterDCTopology(uint32_t no_of_nodes, linkspeed_bps linkspeed, mem_b intra_queuesize, mem_b inter_queuesize,
    ...
    failed_links = 0;
    _link_latency = timeFromUs((uint32_t)1);
...
void FatTreeInterDCTopology::alloc_vectors() {
    ...
    cout << "Number of datacenters: " << number_datacenters << endl;
    cout << "Number of border switches: " << number_border_switches << endl;
    ...
    cout << "Num links to the same border from core: " << _num_links_same_border_from_core << endl;
    cout << "Num links between borders after os_ratio_border: " << _num_links_between_borders << endl;
...
BaseQueue *FatTreeInterDCTopology::alloc_queue_no_ecn(QueueLogger *queueLogger, linkspeed_bps speed, mem_b queuesize,
                                               link_direction dir, int switch_tier, bool tor, double random_drop_perc) {
    switch (_qt) {
    case RANDOM:
        return new RandomQueue(speed, queuesize, *_eventlist, queueLogger, memFromPkt(RANDOM_BUFFER));
    case COMPOSITE: {
        CompositeQueueNoEcn *q = new CompositeQueueNoEcn(speed, queuesize, *_eventlist, queueLogger);
        q->set_random_drop_percentage(random_drop_perc);
        return q;
    }
    case CTRL_PRIO:
        return new CtrlPrioQueue(speed, queuesize, *_eventlist, queueLogger);
...
BaseQueue *FatTreeInterDCTopology::alloc_queue_inter(QueueLogger *queueLogger, linkspeed_bps speed, mem_b queuesize,
                                               link_direction dir, int switch_tier, bool tor, double random_drop_perc) {
    switch (_qt) {
    case RANDOM:
        return new RandomQueue(speed, queuesize, *_eventlist, queueLogger, memFromPkt(RANDOM_BUFFER));
    case COMPOSITE: {
        CompositeQueue *q = new CompositeQueue(speed, queuesize, *_eventlist, queueLogger);
        if (inter_dc_kmin != -1) {
            q->set_kMin(inter_dc_kmin);
            q->set_kMax(inter_dc_kmax);
        }
        return q;
    }
    case CTRL_PRIO:
...
void FatTreeInterDCTopology::init_network() {
        ...
        for (uint32_t tor = 0; tor < NTOR; tor++) {
            ...
            for (uint32_t l = 0; l < link_bundles; l++) {
                ...
                for (uint32_t b = 0; b < _bundlesize[TOR_TIER]; b++) {
                    ...
                    queues_nlp_ns[i][tor][srv][b]->setName("DC" + ntoa(i) + "-LS" + ntoa(tor) + "->DST" + ntoa(srv) + "(" + ntoa(b) +
                                                           ")");
                    // if (logfile) logfile->writeName(*(queues_nlp_ns[tor][srv]));
                    simtime_picosec hop_latency = (_link_latency == 0) ? _link_latencies[TOR_TIER] : _link_latency;
                    pipes_nlp_ns[i][tor][srv][b] = new Pipe(hop_latency, *_eventlist);
                    pipes_nlp_ns[i][tor][srv][b]->setName("DC" + ntoa(i) + "-Pipe-LS" + ntoa(tor) + "->DST" + ntoa(srv) + "(" + ntoa(b) +
                                                          ")");
                    // if (logfile) logfile->writeName(*(pipes_nlp_ns[tor][srv]));

                    // Uplink (server -> ToR (LS))
                    if (_logger_factory) {
                        queueLogger = _logger_factory->createQueueLogger();
                    } else {
                        queueLogger = NULL;
                    ...
                    queues_ns_nlp[i][srv][tor][b]->setName("DC" + ntoa(i) + "-SRC" + ntoa(srv) + "->LS" + ntoa(tor) + "(" + ntoa(b) +
                                                           ")");
                    ...
                    pipes_ns_nlp[i][srv][tor][b]->setName("DC" + ntoa(i) + "-Pipe-SRC" + ntoa(srv) + "->LS" + ntoa(tor) + "(" + ntoa(b) +
                                                          ")");
    ...
    std::unordered_map<int, int> ls_to_us_failures; // Failure map LS to US
    int max_failure_per_switch_ls = NTOR - 1;
    ...
    for (int i = 0; i < number_datacenters; i++) {
        for (uint32_t tor = 0; tor < NTOR; tor++) {
            ...
            for (uint32_t agg = agg_min; agg <= agg_max; agg++) {
                cout << "Creating links between ToR " << tor << " and aggregate switch " << agg << 
                    " -- ToR-to-Agg queue size: " << _queue_up[TOR_TIER] << " B -- Agg-to-ToR queue size: " <<
                    _queue_down[AGG_TIER] << " B." << endl;
                for (uint32_t b = 0; b < _bundlesize[AGG_TIER]; b++) {
                    // Downlink (Agg -> ToR)
                    if (_logger_factory) {
                        queueLogger = _logger_factory->createQueueLogger();
                    ...
                    queues_nup_nlp[i][agg][tor][b]->setName("DC" + ntoa(i) + "-US" + ntoa(agg) + "->LS_" + ntoa(tor) + "(" + ntoa(b) +
                                                            ")");
                    // if (logfile) logfile->writeName(*(queues_nup_nlp[agg][tor]));

                    simtime_picosec hop_latency = (_link_latency == 0) ? _link_latencies[AGG_TIER] : _link_latency;
                    pipes_nup_nlp[i][agg][tor][b] = new Pipe(hop_latency, *_eventlist);
                    pipes_nup_nlp[i][agg][tor][b]->setName("DC" + ntoa(i) + "-Pipe-US" + ntoa(agg) + "->LS" + ntoa(tor) + "(" + ntoa(b) +
                                                           ")");
                    ...
                    queues_nlp_nup[i][tor][agg][b]->setName("DC" + ntoa(i) + "-LS" + ntoa(tor) + "->US" + ntoa(agg) + "(" + ntoa(b) +
                                                            ")");

                    if (rand() % 100 < int(_percentage_link_down * 100)) { // 
                        if (ls_to_us_failures[tor] == 0 || ls_to_us_failures[tor] < max_failure_per_switch_ls) {
                            queues_nlp_nup[i][tor][agg][b]->is_failing = true;
                            ls_to_us_failures[tor] += 1;
                            printf("Failing Link %s\n", queues_nlp_nup[i][tor][agg][b]->_name.c_str());
                        } 
                    }
                    ...
                    pipes_nlp_nup[i][tor][agg][b]->setName("DC" + ntoa(i) + "-Pipe-LS" + ntoa(tor) + "->US" + ntoa(agg) + "(" + ntoa(b) +
                                                           ")");
    ...
    std::unordered_map<int, int> us_to_core_failures; // Failure map US to Core
    int max_failure_per_switch_us = _radix_up[AGG_TIER] / _bundlesize[CORE_TIER] - 1;
    // Upper layer in pod to core!
    if (_tiers == 3) {
        for (int i = 0; i < number_datacenters; i++) {
            for (uint32_t agg = 0; agg < NAGG; agg++) {
                uint32_t podpos = agg % (_agg_switches_per_pod);
                cout << "Creating links between aggregate " << agg << " and all core switches -- Agg-to-Core queue size: " << 
                    _queue_up[AGG_TIER] << " B -- Core-to-Agg queue size: " <<
                    _queue_down[CORE_TIER] << " B." << endl;
                for (uint32_t l = 0; l < _radix_up[AGG_TIER] / _bundlesize[CORE_TIER]; l++) {
                    ...
                    for (uint32_t b = 0; b < _bundlesize[CORE_TIER]; b++) {
                        ...
                        queues_nup_nc[i][agg][core][b]->setName("DC" + ntoa(i) + "-US" + ntoa(agg) + "->CS" + ntoa(core) + "(" + ntoa(b) +
                                                                ")");

                        if (rand() % 100 < _percentage_link_down * 100) { // 
                            if (us_to_core_failures[agg] == 0 || us_to_core_failures[agg] < max_failure_per_switch_ls) {
                                queues_nup_nc[i][agg][core][b]->is_failing = true;
                                us_to_core_failures[agg] += 1;
                                printf("Failing Link %s\n", queues_nup_nc[i][agg][core][b]->_name.c_str());
                            } 
                        }
                        ...
                        simtime_picosec hop_latency = (_link_latency == 0) ? _link_latencies[CORE_TIER] : _link_latency;
                        pipes_nup_nc[i][agg][core][b] = new Pipe(hop_latency, *_eventlist);
                        pipes_nup_nc[i][agg][core][b]->setName("DC" + ntoa(i) + "-Pipe-US" + ntoa(agg) + "->CS" + ntoa(core) + "(" +
                                                               ntoa(b) + ")");
                        // if (logfile)
                        ...
                        queues_nc_nup[i][core][agg][b]->setName("DC" + ntoa(i) + "-CS" + ntoa(core) + "->US" + ntoa(agg) + "(" + ntoa(b) +
                                                                ")");
                        ...
                        pipes_nc_nup[i][core][agg][b]->setName("DC" + ntoa(i) + "-Pipe-CS" + ntoa(core) + "->US" + ntoa(agg) + "(" +
                                                               ntoa(b) + ")");
                        // if (logfile)
    ...
    std::unordered_map<int, int> core_to_border_failures; // Failure map Core to Border
    int max_failure_per_switch_core = number_border_switches - 1;
    // Core to Border
    for (int i = 0; i < number_datacenters; i++) {
        for (uint32_t core = 0; core < NCORE; core++) {
            ...
            for (uint32_t border_sw = 0; border_sw < number_border_switches; border_sw++) {
                ...
                cout << "Creating links between core " << core << " and border switch " << border_sw << " -- speed: " 
                    << speedAsGbps(_downlink_speeds[0]) << " Gbps -- queue size: " << _inter_queuesize << 
                    " B. _num_links_same_border_from_core: " << _num_links_same_border_from_core << endl;

                for (int link_num = 0; link_num < _num_links_same_border_from_core; link_num++) {


                    // UpLinks Queues and Pipes (Core -> border)
                    if (use_inter_dc_ecn) {
                        queues_nc_nborder[i][core][border_sw][link_num] =
                                alloc_queue_inter(queueLogger, _downlink_speeds[0], _inter_queuesize, UPLINK, BORDER_TIER, false);
                    } else {
                        queues_nc_nborder[i][core][border_sw][link_num] =
                    ...
                    if (rand() % 100 < _percentage_link_down * 100) { // 
                        if (core_to_border_failures[core] == 0 || core_to_border_failures[core] < max_failure_per_switch_ls) {
                            queues_nc_nborder[i][core][border_sw][link_num]->is_failing = true;
                            core_to_border_failures[core] += 1;
                            printf("Failing Link %s\n", queues_nc_nborder[i][core][border_sw][link_num]->_name.c_str());
                        } 
                    }
                    ...
                    // DownLinks Queues and Pipes (Border -> core)
                    if (use_inter_dc_ecn) {
                        queues_nborder_nc[i][border_sw][core][link_num] =
                                alloc_queue_inter(queueLogger, _downlink_speeds[0], _inter_queuesize, DOWNLINK, BORDER_TIER, false);
                    } else {
                        queues_nborder_nc[i][border_sw][core][link_num] =
    ...
    std::unordered_map<int, int> border_to_border_failures; // Failure map Border to Border
    int max_failure_per_switch_border = number_border_switches - 1;
    ...
    for (uint32_t border_l = 0; border_l < number_border_switches; border_l++) {
        for (uint32_t border_u = 0; border_u < number_border_switches; border_u++) {
            ...
            cout << "Creating links between border switches " << border_l << " and " << border_u << " -- speed: " 
                << speedAsGbps(_downlink_speeds[0]) << " Gbps -- queue size: " << _inter_queuesize << 
                " B. _num_links_between_borders: " << _num_links_between_borders << endl;
            ...
            for (int link_num = 0; link_num < _num_links_between_borders; link_num++) {
                ...
                if (use_inter_dc_ecn) {
                    queues_nborderl_nborderu[border_l][border_u][link_num] =
                            alloc_queue_inter(queueLogger, _downlink_speeds[0], _inter_queuesize, UPLINK, BORDER_TIER, false, random_drop_perc);
                } else {
                    queues_nborderl_nborderu[border_l][border_u][link_num] =
                            alloc_queue_no_ecn(queueLogger, _downlink_speeds[0], _inter_queuesize, UPLINK, BORDER_TIER, false, random_drop_perc);
                }

                if (rand() % 100 < _percentage_link_down * 100) { // 
                    if (border_to_border_failures[border_l] == 0 || border_to_border_failures[border_l] < max_failure_per_switch_ls) {
                        queues_nborderl_nborderu[border_l][border_u][link_num]->is_failing = true;
                        border_to_border_failures[border_l] += 1;
                        printf("Failing Link %s\n", queues_nborderl_nborderu[border_l][border_u][link_num]->_name.c_str());
                    } 
                }
                ...
                if (use_inter_dc_ecn) {
                    queues_nborderu_nborderl[border_u][border_l][link_num] =
                            alloc_queue_inter(queueLogger, _downlink_speeds[0], _inter_queuesize, DOWNLINK, BORDER_TIER, false, random_drop_perc);
                } else {
                    queues_nborderu_nborderl[border_u][border_l][link_num] =
                            alloc_queue_no_ecn(queueLogger, _downlink_speeds[0], _inter_queuesize, DOWNLINK, BORDER_TIER, false, random_drop_perc);
                }
