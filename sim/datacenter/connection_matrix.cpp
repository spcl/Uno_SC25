...
void ConnectionMatrix::setPermutation(uint32_t conn, uint32_t rack_size) {
    ...
    for (uint32_t i = 0; i < conn; i++) {
        ...
        if (rack_count < N) {
            ...
            // if
            // (rack_load[perm_tmp[pos]/rack_size]>rack_load[perm_tmp[pos2]/rack_size])
            // pos = pos2;
        }
...
void ConnectionMatrix::setPermutationShuffle(uint32_t conns) {
    ...
    // OK, we should have a shuffled set of N destinations, where no src sends
    // to itself we now use the first "conns" sources from our shuffled "srcs"
    // vector
...
bool ConnectionMatrix::load(istream &file) {
    ...
    &c->size) < 4) { perror("Connection matrix load error"); fprintf(stderr,
    "Incorrect syntax for connection %d\n", i); return false;
        }
    ...
    do {
        ...
        if (dstix != string::npos) {
            ...
            if (c->start != TRIGGER_START && c->trigger) {
                cerr << "Error: both start time and trigger specified for flow "
                        "at line "
                     << linecount << endl;
                exit(1);
            }
