...
MprdmaSrc::~MprdmaSrc() {
    ...
    if (COLLECT_DATA) {
        ...
        for (const auto &p : _list_timeout) {
            MyFileTimeout << get<0>(p) << "," << get<1>(p) << std::endl;
        }
        MyFileTimeout.close();
...
void MprdmaSrc::updateParams() {
    ...
    MyFile << "Target QDELAY (us)," << TARGET_QDELAY / 1000000 << std::endl;
...
void MprdmaSrc::processNack(UecNack &pkt) {
    ...
    if (!LCP_USE_TRIMMING) {
        std::cout << "MprdmaSrc: not using trimming, how are we getting nacks?" << endl;
        exit(1);
    }
...
void MprdmaSrc::startflow() {
    ...
    if (!flow_started) {
        ...
        flow_started = true;
        cout << "MprdmaSrc::Starting flow (ID: " << _flow.flow_id() << ", size: " << _flow_size << ") at time " << timeAsUs(_flow_start_time) << "us" << endl;
    }
...
void MprdmaSrc::retransmit_packet() {
    _list_timeout.push_back(std::make_tuple(eventlist().now() / 1000, aggregate_timedout));
    _rtx_pending = false;
    for (std::size_t i = 0; i < _sent_packets.size(); ++i) {
