...
class Packet {
  ...
  public:
    ...
    int subflow_number = 0;

    // EC
    unsigned long parity_id = 0;
    simtime_picosec ec_wait_time = 0;
    unsigned long highest_completed_parity_id = 0;
    unsigned long completed_batch_parity_id = 0;
    unsigned long nack_parity_id = 0;
    ...
    inline void set_queue_idx(int value) { _queue_idx = value; }
    inline int queue_idx() { return _queue_idx; }

    virtual void strip_payload() {
        assert(!_is_header);
        // std::cout << "Stripping payload" << endl;
        _is_header = true;
        _is_trim = true;
    };
    virtual void bounce();
  ...
  protected:
    ...
    // queue_idx: indicates which queue the message should enter in the network
    int _queue_idx = -1;
};
...
class PacketSink {
  public:
    ...
    virtual ~PacketSink() {
        for (auto it = rcvd_batches_map.begin(); it != rcvd_batches_map.end(); it++) {
            delete it->second;
        }
        rcvd_batches_map.clear();
    }
    ...
    string ec_handler = "dst";  // either src or dst: which entity is in charge of enforcing EC?
    unsigned int parity_group_size = 0;
    unsigned int parity_correction_capacity = 0; // how many lost packets from a group can be corrected by parity
    unsigned long highest_completed_parity_id = 0;  // highest parity ID completed sequentially
    map<unsigned long, unsigned int> rcvd_parity_ids_map;  // maps acked parity id --> parity count
    ...
    unsigned long get_parity_id(uint64_t seqno, uint16_t _mss) {
        // base parity id is 1
        unsigned long parity_id = 1 + uint64_t(seqno * 1.0 / (parity_group_size * _mss));
        if (parity_id <= 0) {
            cout << "send_parity_id should always stay > 0" << endl;
            abort();
        }
        return parity_id;
    }
    ...
    class Batch {
        public:
            Batch(unsigned long _parity_id) { parity_id = _parity_id; }
            virtual ~Batch() {}

            unsigned long parity_id;
            unsigned long parity_cnt = 0;
            simtime_picosec parity_exp_time = 0;
            set<uint64_t> rcvd_seqno_set;
            simtime_picosec ec_wait_time = 0;
    };
    map<unsigned long, Batch*> rcvd_batches_map;
};
