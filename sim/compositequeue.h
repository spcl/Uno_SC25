...
class CompositeQueue : public Queue {
  public:
    ...
    mem_b _queuesize_high, _current_queuesize_phatom;
    ...
    static bool _phantom_observe;

    
    std::unordered_map<int, int> seen_pkts;
    std::unordered_map<int, int> dropped_pkts;

    std::unordered_map<int, int> seen_acks;

    void set_kMin(int value) { _kmin_from_input = value; }

    void set_kMax(int value) { _kmax_from_input = value; }
    ...
    static void set_use_both_queues_true() { _use_both_queues_true = true; }
    ...
    simtime_picosec _decrease_normal_next = 0;
    simtime_picosec _draining_time_phantom = 0;
    static bool _phantom_in_series;
    int _kmin_from_input = 20;
    int _kmax_from_input = 80;
  ...
  protected:
    ...
    bool decide_ECN(int queue_idx = -1);
    ...
    // Fow now, this is just two elements for inter- and intra-
    mem_b _queuesize_low[2];
    CircularBuffer<Packet *> _enqueued_low_list[2];
    CircularBuffer<Packet *> _enqueued_high;

    // Added for low queues list
    void reset_queuesize_low() { _queuesize_low[0] = _queuesize_low[1] = 0; }
    bool all_enqueued_low_queues_empty() { return _enqueued_low_list[0].empty() && _enqueued_low_list[1].empty(); }
    simtime_picosec get_enqueue_low_min_drain_time() {
        if (_enqueued_low_list[0].empty())
            return drainTime(_enqueued_low_list[1].back());
        if (_enqueued_low_list[1].empty())
            return drainTime(_enqueued_low_list[0].back());
        return min(drainTime(_enqueued_low_list[0].back()), drainTime(_enqueued_low_list[1].back()));
    }
    Packet* popLowQueue();
    void check_low_queue_sizes() {
        if (_queuesize_low[0] < 0 || _queuesize_low[1] < 0) {
            cout << "_queuesize_low[0]: " << _queuesize_low[0] << endl;
            cout << "_queuesize_low[1]: " << _queuesize_low[1] << endl;
            abort();
        }
    };
    uint64_t weight_cnt = 0;
};
