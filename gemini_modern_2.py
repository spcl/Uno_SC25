...
for ecn_value in ecn_absolute_values:
    ...
    cmd = (
        ...
        f"-topology interdc -os_border 128 -strat rand -linkspeed 400000 "
        f"-topo lcp/configs/topos/fat_tree_400Gbps_gemini.topo -tm lcp/configs/tms/simple/gemini_interdc.cm "
        ...
        f"-mdRTT 0.0003515625 -interdcDelay 4865000 -forceQueueSize {queue_Size * 10} -end_time 80000 -mtu 1500 -dctcp -forceKmin {ecn_value * mtu_size} -forceKmax {ecn_value * mtu_size} -forceCWND 15000 > tmp.tmp"
    )
    print(f"Running: {cmd}")
