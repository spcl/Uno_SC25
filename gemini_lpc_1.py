...
ecn_absolute_values = [20, 40, 80, 160, 320]
ecn_absolute_values = [20, 40, 80, 160, 320]
...
base_rtt = 200              # in microseconds
message_size = (10000000 * 14)          # in bytes (adjust as needed)
# Calculate ideal completion time: transmission delay = message_size (bytes) / (125 bytes/us at 1Gbps)
ideal_fct = base_rtt + message_size / (125 * 1)  
...
for ecn_value in ecn_absolute_values:
    ...
    cmd = (
        ...
        f"-topology interdc -os_border 1 -strat rand -linkspeed 1000 "
        f"-topo lcp/configs/topos/fat_tree_100Gbps_gemini.topo -tm lcp/configs/tms/simple/gemini_intradc.cm "
        f"-noFi -noRto -queueSizeRatio 0.1 -IntraFiT 10 -InterFiT 1280 -kmin {ecn_relative_values} -kmax {ecn_relative_values} "
        f"-interKmin {ecn_relative_values} -interKmax {ecn_relative_values} -ecnAlpha 0.5 -usePacing 0 -lcpK 6 -interEcn "
        f"-mdRTT 0.0003515625 -interdcDelay 886500 -forceQueueSize {queue_Size * 100} -end_time 8000 -forceKmin {ecn_value * mtu_size} -forceKmax {ecn_value * mtu_size} > tmp.tmp"
    )
    print(f"Running: {cmd}")
...
ax2.set_ylim(0,20)  # Left y axis limit only.
