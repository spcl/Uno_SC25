...
def plot_results(agg_results):
    ...
    for i, variant in enumerate(algo_keys):
        ...
        avg_err = res["avg_err"]  # now standard deviation
        avg_min = res["avg_min"]
        avg_max = res["avg_max"]
        p99_mean = res["p99_mean"]
        p99_err = res["p99_err"]  # standard deviation for p99
        p99_min = res["p99_min"]
        p99_max = res["p99_max"]

        values = [avg_mean, p99_mean]
        errors = [avg_err, p99_err]  # error bars equal to the standard deviation
        ...
        for bar in bars:
            ...
            """ ax.annotate(f"{height:.1f}",
                        xy=(bar.get_x() + bar.get_width()/2, 22),
                        xytext=(0, 3),
                        textcoords="offset points",
                        ha="center", va="bottom",
                        fontsize=8.5, color="black") """
        # Add markers at the min and max points:
        # For Avg FCT (first metric)
    ...
    if not os.path.exists(results_folder):
        os.makedirs(results_folder)
    plt.savefig("failuire_legend.png", dpi=300, bbox_inches="tight")
    plt.savefig("failuire_legend.pdf", dpi=300, bbox_inches="tight")
    plt.show()
