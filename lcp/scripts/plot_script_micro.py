...
def main(args):
    ...
    queue_phantom_df = get_df('queue_phantom', ['Time', 'Occupancy', 'Ecn Min Thresh', 'Ecn Max Thresh', 'Capacity'], max_values=MAX_VALUE)
    ...
    debug_fig2 = make_subplots(rows=9, cols=1, specs=[[{}], [{}], [{}], [{}], [{}], [{}], [{}], [{}], [{}]], shared_xaxes=False, vertical_spacing=0.1, subplot_titles=('EWMA', 'Nack', 'CWND Reduction Factor', 'Rate2', 'Goodput (bps)', 'Rate CDF', '', ''))
...
def print_fct_info():
    ...
    # 100 Gbps
    link_bps = 10**(11)
    base_rtt_intra_s = 14 * 10**(-6)
    base_rtt_inter_s = 112 * 10**(-6)   # 8x
    base_rtt_inter_s = 448 * 10**(-6)   # 32x
    base_rtt_inter_s = 1792 * 10**(-6)   # 128x
    base_rtt_inter_s = 7168 * 10**(-6)   # 512x

    param_df = get_df("params", ['Parameter', 'Value'])
    baremetal_rtt_df = param_df[param_df['Parameter'] == 'Baremetal RTT (us)']
    ...
    # node_name --> fct
    intra_fct_dict = dict()
    intra_slowdown_dict = dict()
    inter_fct_dict = dict()
    inter_slowdown_dict = dict()
    intra_fct_list = [0]
    intra_slow_down_list = [0]
    inter_fct_list = [0]
    inter_slow_down_list = [0]
    for index, row in fct_df.iterrows():
        flow_size_sub_df = flow_size_df.loc[flow_size_df['Node'] == row['Node']]
        f_size_b = flow_size_sub_df['flow_size'].iloc[0] * 8.0
        baremetal_rtt_sub_df = baremetal_rtt_df.loc[baremetal_rtt_df['Node'] == row['Node']]
        base_rtt_s = None
        if (len(baremetal_rtt_sub_df) > 0):
            # base_rtt is in us
            base_rtt_us = float(baremetal_rtt_sub_df.iloc[0]['Value'])
            base_rtt_s = base_rtt_us * 1.0 / (10**6)
        # fct is in us
        fct_s = row['FCT'] * 1.0 / (10**6)
        # print(f'fsize: {f_size_b} b, base_rtt: {base_rtt_s} s, ideal fct: {ideal_fct} s, fct: {fct_s} s, slow_down: {slow_down}')
        node_name_splitted = row['Node'].split('_')
        source = int(node_name_splitted[1])
        dst = int(node_name_splitted[2])
        if int(source / SERVERS_PER_DC) == int(dst / SERVERS_PER_DC):
            ...
            if (base_rtt_s is None):
                base_rtt_s = base_rtt_intra_s
            base_rtt_intra_s = base_rtt_s
            ideal_fct = base_rtt_s + (f_size_b * 1.0 / link_bps)
            slow_down = max(1, fct_s / ideal_fct)
            intra_fct_dict[row['Node']] = row['FCT']
            intra_slowdown_dict[row['Node']] = slow_down
        else:
            # inter-DC flow
            if (base_rtt_s is None):
                base_rtt_s = base_rtt_inter_s
            base_rtt_inter_s = base_rtt_s
            ideal_fct = base_rtt_s + (f_size_b * 1.0 / link_bps)
            slow_down = max(1, fct_s / ideal_fct)
            inter_fct_dict[row['Node']] = row['FCT']
            inter_slowdown_dict[row['Node']] = slow_down
    if (len(intra_fct_dict) > 0):
        intra_fct_list = list(intra_fct_dict.values())
        intra_slow_down_list = list(intra_slowdown_dict.values())
    if (len(inter_fct_dict) > 0):
        inter_fct_list = list(inter_fct_dict.values())
        inter_slow_down_list = list(inter_slowdown_dict.values())
    all_fct_list = intra_fct_list + inter_fct_list
    all_slowdown_list = intra_slow_down_list + inter_slow_down_list
    print(f'Link BW: {link_bps / 10**9} Gbps')
    print(f'All FCT ({len(all_fct_list)} flows) --> Mean: {np.mean(all_fct_list)} us, p99: {np.percentile(all_fct_list, 99)} us, max: {max(all_fct_list)} us')
    print(f'Intra FCT ({len(intra_fct_list)} flows) --> Mean: {np.mean(intra_fct_list)} us, p99: {np.percentile(intra_fct_list, 99)} us, max: {max(intra_fct_list)} us')
    print(f'Inter FCT ({len(inter_fct_list)} flows) --> Mean: {np.mean(inter_fct_list)} us, p99: {np.percentile(inter_fct_list, 99)} us, max: {max(inter_fct_list)} us')
    print()
    print(f'base_rtt_intra_s: {base_rtt_intra_s}, base_rtt_inter_s: {base_rtt_inter_s}')
    print(f'All slow down ratio --> Mean: {np.mean(all_slowdown_list)}, p99: {np.percentile(all_slowdown_list, 99)}')
    print(f'Intra slow down ratio --> Mean: {np.mean(intra_slow_down_list)}, p99: {np.percentile(intra_slow_down_list, 99)}')
    print(f'Inter slow down ratio --> Mean: {np.mean(inter_slow_down_list)}, p99: {np.percentile(inter_slow_down_list, 99)}')
...
if __name__ == "__main__":
    ...
    parser.add_argument("--fct_only", action='store_true', help="Just report fct", default=False)
